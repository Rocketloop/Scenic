...
class RegulatedControlAction(SteeringAction):
    ...
    def __init__(self, throttle: float, steer: float, past_steer: float, max_throttle: float = 0.5,
                 max_brake: float = 0.5, max_steer: float = 0.8, max_steer_per_timestep=0.1):
        if throttle > 0:
        ...
        if steer > past_steer + max_steer_per_timestep:
            steer = past_steer + max_steer_per_timestep
        elif steer < past_steer - max_steer_per_timestep:
            steer = past_steer - max_steer_per_timestep
